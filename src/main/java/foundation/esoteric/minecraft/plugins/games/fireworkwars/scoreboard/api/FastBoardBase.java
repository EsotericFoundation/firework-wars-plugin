...
public abstract class FastBoardBase<T> {
     ...
     * @throws IllegalStateException    if {@link #delete()} was call before
     */
    public synchronized void updateScores(Collection<? extends T> texts) {
    ...
    private void checkLineNumber(int line, boolean checkInRange, boolean checkMax) {
        ...
        if (checkMax && line >= COLOUR_CODES.length - 1) {
    ...
    protected T getLineByScore(List<? extends T> lines, int score) {
    ...
    protected void sendScorePacket(int score, ScoreboardAction action) throws Throwable {
        ...
        setField(packet, String.class, COLOUR_CODES[score], 0); // Player Name
    ...
    protected void sendTeamPacket(int score, TeamMode mode, T prefix, T suffix)
        ...
        if (mode == TeamMode.CREATE) {
            setField(packet, Collection.class, Collections.singletonList(COLOUR_CODES[score])); // Players in the team
        }
